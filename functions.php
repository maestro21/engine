...
function getLang(){
	global $labels;
	$lang = getVar('lang',G('deflang','en')); setVar('lang',$lang);
	$tmp = file("lang/$lang.txt"); // print_r($tmp);
	foreach($tmp as $s){
...
function loadClass($class){
	if(file_exists(BASE_PATH.'classes/'.$class)){
		require_once(BASE_PATH.'classes/'.$class);
		return new $class();
	}else
		return false;
}
